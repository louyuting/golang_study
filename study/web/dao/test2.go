...
func main() {
	client, err := DialWithPasswd("nycjump01.fwmrm.net:22", "ytlou", "Sim520minjun,.")
	if err != nil {
		panic(err)
	}
	...
	if db, err := sql.Open("mysql", fmt.Sprintf("%s:%s@mysql+tcp(%s)/%s", "qa", "Qa0602@@", "OLTPdb1a.fwmrm.net:3306", "fwmrm_oltp")); err == nil {
		fmt.Printf("Successfully connected to the db\n")
		if rows, err := db.Query("SELECT * FROM lu_country"); err == nil {
			for rows.Next() {
				var uid int
				var name string
